  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      const response = await axios.patch(
        `${API_Endpoint}/business/auth/${userId}`,
        formData,
        {
          headers: {
...
const DeactivateDailog = ({ open, handleClose }) => {
  ...
  useEffect(() => {
    ...
    // Check if the 'user' object exists and if it has a 'BusinessEmail' property
    if (user && user.BusinessEmail) {
      setBusinessEmail(user.BusinessEmail); // Update the state variable with the value from localStorage
    }
  }, []);
...
const ReportDailog = ({ open, handleClose }) => {
  ...
  const submitReport = async () => {
    // get businessId from local storage

    try {
      const response = await axios.post(`${API_Endpoint}/business/report`, {
        business_ID,
        description,
      });
